...
class EndpointValidator
{
    ...
    {
        $lengthHelper = strpos($this->validatorDataCollector->url, 'api/v1/') + 7;
        if (strpos($this->validatorDataCollector->url, 'api/v1/') === false) {
            $lengthHelper = strpos($this->validatorDataCollector->url, 'api/v1') + 7;
        }

        return substr($this->validatorDataCollector->url, 0, $lengthHelper);
    }
